		...
		EM_ASM((function() {
			...
			function installMobileStyles() {
				...
				style.textContent = [
					...
					'@media (pointer: coarse) and (orientation: landscape), (max-width: 900px) and (orientation: landscape) { body.ggn-mobile-ready { --ggn-cell: max(18px, min(28px, calc((100dvh - 52px) / 10), calc((100vw - 420px) / 12))); --ggn-controls-height: 0px; --ggn-landscape-manual-height: 40px; } body.ggn-mobile-ready #ggn-page-links { top: max(4px, env(safe-area-inset-top)); left: 50%; right: auto; transform: translateX(-50%); } body.ggn-mobile-ready div.emscripten_border { width: 100vw; height: 100dvh; box-sizing: border-box; padding-left: calc(var(--ggn-pad-width) + max(var(--ggn-edge), env(safe-area-inset-left)) + 8px); padding-right: calc(var(--ggn-pad-width) + max(var(--ggn-edge), env(safe-area-inset-right)) + 8px); padding-top: calc(var(--ggn-landscape-manual-height) + env(safe-area-inset-top)); align-items: center; justify-content: center; } body.ggn-mobile-ready canvas.emscripten { width: min(100%, calc((100dvh - var(--ggn-landscape-manual-height) - env(safe-area-inset-top)) * 1.333333)); height: auto; max-height: calc(100dvh - var(--ggn-landscape-manual-height) - env(safe-area-inset-top)); aspect-ratio: 4 / 3; } #ggn-touch-controls { inset: 0; height: auto; } #ggn-touch-controls .ggn-pad { bottom: auto; top: 50%; transform: translateY(-50%); } }',
