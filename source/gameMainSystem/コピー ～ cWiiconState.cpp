#include "cInputChild.h"
#include "cInput.h"
#include "cEnvironment.h"
#include "Main.h"

cInputChild::cInputChild(void)
{
}
...
void cInputChild::InitWiicon(CWiimote * pWiicon)
{
	WiiconDev = pWiicon;
...
//子クラスオーバーロード用
void cInputChild::ResetJudgeCustum(int playerNum)
{}
//子クラスオーバーロード用
void cInputChild::InitCustum(CWiimote * pWiicon)
{}


//毎回呼ばれて情報更新を各コントローラー別に行う
int cInputChild::ResetAdvanceInput(int num)
{
	//実行順を変えないように
...
c4DVector cInputChild::decodePointer(c4DVector p_raw)
{
...
void cInputChild::GetWiiToWorldDegree()
{
...
c4DVector cInputChild::decodeMotion(c4DVector m_raw)
{
...
void cInputChild::makePlumpdownline()
{
...
c4DVector cInputChild::motionToPointer(c4DVector m)
{
...
c4DVector cInputChild::realPointer()
{
...
void cInputChild::setForceFeedBack(int frame)
{
	if(FFBframe_count <= -5)
		FFBframe_count = frame;
}
void cInputChild::setForceFeedBackForcibly(int frame)
{
	FFBframe_count = frame;
}

void cInputChild::ForceFeedBackReset()
{
	if(FFBframe_count > 0)
	{
		if(!ForceFeedOn)
