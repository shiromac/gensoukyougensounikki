...
void EffectFunctions::ボスエフェクト集中(const double placeX,const double placeY,const double placeZ)
{
	...
	for(i=0;i<200;i++)
	{
		...
		cRenderMatrix mat;
		cRenderMatrixRotationZ(&mat, (float)(random_for_effect()*2*3.14159));
		cRenderVec3TransformCoord(&pos,&pos,&mat);
...
void EffectFunctions::弾幕体当たり(const double placeX,const double placeY, pcCharacter pchara, const int aspect)
{
	...
	cRenderTexture* m_pTexture = g_GameEnv.m_GlobalResourse->getTextureFromFile(sg_pDungeonSystem->pDevice_D3D,path);
