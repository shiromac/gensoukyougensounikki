...
function startSpell()
    ...
    CharacterThrow(boss,boss, LandformPlace(bossFixPlaceX,bossFixPlaceY), 0, YES, 3.0);
end
...
function LazerMove(timeCount, x)
    pland = landformSearchObstacleWithRange(LandformPlace(x,MAPHEIGHT/2), ASPECT_UP, MAPHEIGHT,true,false,false,false);
    pland = LandformNearby(pland,ASPECT_DOWN,1);
    lazerSummon(pland, ASPECT_DOWN, 50, ValiableConstant3, CHARACTER_FORSE_ENEMY, Color(255,255,81,137), 1, timeCount, false, true);
end
...
function danmaku_setting()
    ...
    if (spellTurnCount % 4) == 0 and (spellTurnCount % LazerCountMax) ~= 0
    then
    ...
    if  (spellTurnCount % 4) == 0 and ((spellRestTurn / spellTurnLimitCount < 0.4) or (boss.HP / boss.maxHP < 0.4))
    then
