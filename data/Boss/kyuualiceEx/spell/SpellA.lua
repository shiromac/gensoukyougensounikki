...
function startSpell()
    ...
    CharacterThrow(boss,boss, LandformPlace(20, 12), 0, YES, 3.0);
end
...
function gensezan()    
    ...
    lazerSummon(landformAroundOfCharacter(boss, 0, 1), boss.aspect,distance, ValiableConstant3, CHARACTER_FORSE_ENEMY, Color(255,255,255,137), 0, 0, true, true);
...
function danmaku_setting()
    ...
    len = {1,3,5,7,9,11,12,13,13,14,14,14,13,13,12,11,9,7,5,3,1}
    ...
    then
        ...
        do
            ...
            then
                lazerSummon(LandformPlace(8, i-13+(lazerCount % lazerThreshold)), ASPECT_RIGHT, len[i], ValiableConstant3, CHARACTER_FORSE_ENEMY, Color(255,0,0,255), 0, 1, false, true);
            end    
        end
    else
        for i = 1, 21 
        do
            if(i-13+(lazerCount % lazerThreshold) >= 8) and  (i-13+(lazerCount % lazerThreshold) <= 32)
            then
                lazerSummon(LandformPlace(32, i-13+(lazerCount % lazerThreshold)), ASPECT_LEFT, len[i], ValiableConstant3, CHARACTER_FORSE_ENEMY, Color(255,0,0,255), 0, 1, false, true);
            end    
        end
    end
    
    lazerCount=lazerCount+3;
    return true;
end
