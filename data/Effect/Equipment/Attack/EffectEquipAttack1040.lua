...
-- fumikiri
fumikiri_life = 16
fumikomi_distance = isValidAttack * 1.5 * sqrt((fromPlaceX - toPlaceX)*(fromPlaceX - toPlaceX) + (fromPlaceY - toPlaceY)*(fromPlaceY - toPlaceY))/ distance 

chara_anime = AnimeChip_Chara(attacker,fumikiri_life)
hab:addAnimeChip(chara_anime)
chara_anime: setWorldRotation(direction)
--chara_anime.position: addCascade(PositionPerGrid(0, 0, 0), PositionPerGrid(0, -0.1, 0), fumikiri_life*0.5)
chara_anime.position: addCascade(PositionPerGrid(0, -0.1, 0), PositionPerGrid(0, -0.8*fumikomi_distance, 0), PositionPerGrid(0, 0, 0), fumikiri_life)
chara_anime.yawing: addCascade(0, -45, -90, -135, fumikiri_life*0.5)
chara_anime.yawing: addCascade(-135, -180, -225, -270, fumikiri_life*0.5)
--

-- sound
sound = AnimeChip_Sound("swing1040.wav",PositionPerGrid(toPlaceX,toPlaceY,0),1.0)
...
aniBelt11.position: addCascade(PositionPerGrid(0, 0.5, 0.6), life_swing)
...
for k = 0, belt_point_num do
	pos = PositionPerGrid(0.6, 0, 0)	-- ベースの位置ベクトル(先端位置)
	rotAxis = PositionPerGrid(0, 0, 1.0)	-- 回転軸
	
	angle =  ( (k/belt_point_num)*500 + 290 ) * -MATH_PI/180	-- 回転角度
...
aniBelt21.position: addCascade(PositionPerGrid(0, 0.2, 0.6), life_swing)
...
for k = 0, belt_point_num do
	...
	angle =  ( (k/belt_point_num)*500 + 30 ) * MATH_PI/180	-- 回転角度
	D3DXVec3Rotate( pos, rotAxis, angle, pos )			-- 位置ベクトルを回転
	
	aniBelt21.rightEdge: push_back(VariationValue_PositionPerGrid(pos*0.3))	-- 手前
	aniBelt21.leftEdge: push_back(VariationValue_PositionPerGrid(pos))	-- 奥
end
